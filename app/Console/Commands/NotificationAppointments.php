...
class NotificationAppointments extends Command
{
    ...
    public function handle()
    {
        ...
        $simulet_hour_number = date("2024-05-29 16:01:15"); //strtotime(date("2024-05-29 15:10:00"));  //
        $appointments = Appointment::whereDate("date_appointment","2024-05-29")//now()->format("Y-m-d")
                                    ->where("status",1)
                                   // ->where("cron_state",1)
                                    ->get();
       ...
       // error_log("Antes de entrar aqui");
        foreach ($appointments as $key => $appointment) {
            ...
            $hour_start = strtotime(Carbon::parse("2024-05-29"." ".$hour_start)->subHour());
            $hour_end = strtotime(Carbon::parse("2024-05-29"." ".$hour_end)->subHour());
